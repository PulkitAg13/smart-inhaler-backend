...
def upload_sensor_data():
    ...
    required_fields = ["user_id", "air_quality", "pm25", "so2_level", "no2_level", "co2_level", "humidity", "temperature"]
...
@routes.route('/use-inhaler/<int:user_id>', methods=['POST'])
def use_inhaler(user_id):
    ...
    db.session.commit()
    return jsonify({'message': 'Inhaler usage recorded', 'usage_count': inhaler_usage.usage_count}), 200
...
def submit_quiz():
    ...
    stored_questions = [
        ...
        "Which of the following commonly trigger your symptoms? (Select all that apply)",
    ...
    for question, answer_text in answers.items():
        if question in stored_questions:
            new_response = QuizResponse(user_id=user_id, question=question, answer=answer_text)
...
def send_data_to_ai(user_id):
    ...
    try:
        ...
        risk_score = ai_result.get("asthma_risk_score", 0)  # Default risk score = 0 if missing
        ...
        return jsonify({'risk_score': risk_score, 'message': 'AI risk score received'}), 200

    except requests.exceptions.RequestException as e:
        return jsonify({'error': 'Failed to connect to AI model', 'details': str(e)}), 500